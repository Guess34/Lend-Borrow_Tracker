...
public class GroupService
{
	...
	public String useInviteCode(String code, String playerName)
	{
		...
		// Check shared invite key (for codes generated by other accounts)
		...
		if (relaySyncService != null)
		{
			try
			{
				String relayJson = relaySyncService.lookupInviteCode(trimmedCode);
				if (relayJson != null && !relayJson.isEmpty())
				{
					LendingGroup relayGroup = gson.fromJson(relayJson, LendingGroup.class);
					if (relayGroup != null && relayGroup.getId() != null)
					{
						...
						// Consume the code on the relay server
	...
	public String generateSingleUseInviteCode(String groupId)
	{
		...
		if (group != null)
		{
			...
			saveGroups();
			// Store in shared config so other accounts on same machine can look up this code
			configManager.setConfiguration(CFG_GROUP, INVITE_KEY_PREFIX + code, gson.toJson(group));
			// Publish to relay server for cross-machine invite code lookup
			if (relaySyncService != null)
			{
				relaySyncService.publishInviteCode(code, groupId, gson.toJson(group));
			}
			return code;
		}
		return null;
